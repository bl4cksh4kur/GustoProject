...
class User implements UserInterface
{
     ...
     * @Assert\Regex(pattern="/^[a-zA-Z0-9 ]+$/", match=true, message="Votre prénom comporte des caractères invalides")
     */
    private $firstname;

    /**
     * @ORM\Column(type="string", length=255)
     * @Assert\Regex(pattern="/^[a-zA-Z0-9 ]+$/", match=true, message="Votre nom comporte des caractères invalides")
     */
    private $lastname;
    ...
    /**
     * @ORM\OneToMany(targetEntity=BookingPlace::class, mappedBy="user")
     */
    ...
    public function removeBookingPlace(BookingPlace $bookingPlace): self
    {
        if ($this->bookingPlaces->removeElement($bookingPlace)) {
            // set the owning side to null (unless already changed)
            if ($bookingPlace->getUser() === $this) {
                $bookingPlace->setUser(null);
            }
        }
